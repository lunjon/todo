    ...
    T: Item,
{
    ...
    /// Only push the item if it not already exists.
    pub fn push_not_exists(&mut self, item: T) {
    ...
    pub fn remove(self, v: &T) -> Self {
        Self::new(self.0.into_iter().filter(|item| item != v).collect())
    }
}
